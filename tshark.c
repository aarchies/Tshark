...
static bool capture_input_new_file(capture_session *cap_session, char *new_file);
static void capture_input_new_packets(capture_session *cap_session, int to_read);
static void capture_input_drops(capture_session *cap_session, uint32_t dropped, const char *interface_name);
static void capture_input_error(capture_session *cap_session, char *error_msg, char *secondary_error_msg);
static void capture_input_cfilter_error(capture_session *cap_session, unsigned i, const char *error_message);
...
static bool process_packet_single_pass(capture_file *cf, epan_dissect_t *edt, int64_t offset, wtap_rec *rec, unsigned tap_flags);
// static cf_status_t cf_write(capture_file *cf, const char *binary, const size_t len);
...
static bool
must_do_dissection(dfilter_t *rfcode, dfilter_t *dfcode, char *volatile pdu_export_arg)
{
...
static GList *capture_opts_get_interface_list(int *err, char **err_str)
{
...
int main(int argc, char *argv[])
{
    ...
    ws_info("测试总耗时: %f s", cpu_time_used);
    ...
    static const struct ws_option long_options[] = {
        ...
        LONGOPT_CAPTURE_COMMON
            LONGOPT_DISSECT_COMMON
                LONGOPT_READ_CAPTURE_COMMON{"print", ws_no_argument, NULL, 'P'},
...
static epan_t *tshark_epan_new(capture_file *cf)
{
...
capture_input_new_packets(capture_session *cap_session, int to_read)
{
    ...
    if (do_dissection)
    {
        ...
        while (to_read-- && cf->provider.wth)
        {
            ...
            {
                ret = process_packet_single_pass(cf, edt, data_offset, &rec, tap_flags);
            }
            if (ret != false)
            {
...
#else
static void capture_cleanup(int signum _U_)
{
    /* tell the capture child to stop */
    sync_pipe_stop(&global_capture_session);
...
static bool
process_packet_first_pass(capture_file *cf, epan_dissect_t *edt, int64_t offset, wtap_rec *rec)
{
    frame_data fdlocal;
    uint32_t framenum;
    bool passed;
...
process_cap_file_first_pass(capture_file *cf, int max_packet_count, int64_t max_byte_count, int *err, char **err_info)
{
    wtap_rec rec;
...
static bool
process_packet_second_pass(capture_file *cf, epan_dissect_t *edt, frame_data *fdata, wtap_rec *rec, unsigned tap_flags _U_)
{
    column_info *cinfo;
    bool passed;
...
static pass_status_t
process_cap_file_second_pass(capture_file *cf, wtap_dumper *pdh, int *err, char **err_info, volatile uint32_t *err_framenum, int max_write_packet_count)
{
    wtap_rec rec;
    int framenum = 0;
...
static pass_status_t
process_cap_file_single_pass(capture_file *cf, wtap_dumper *pdh, int max_packet_count, int64_t max_byte_count, int max_write_packet_count, int *err, char **err_info, volatile uint32_t *err_framenum)
{
    wtap_rec rec;
...
process_cap_file(capture_file *cf, char *save_file, int out_file_type, bool out_file_name_res, int max_packet_count, int64_t max_byte_count, int max_write_packet_count, wtap_compression_type compression_type)
{
...
static bool
process_packet_single_pass(capture_file *cf, epan_dissect_t *edt, int64_t offset, wtap_rec *rec, unsigned tap_flags _U_)
{
    frame_data fdata;
    column_info *cinfo;
    bool passed;
...
static void
write_field_data(proto_node *node, GArray *field_array, const char *protocol_name)
{
    ...
    gchar *local_value_string = fvalue_to_string_repr(NULL, fi->value, FTREPR_DISPLAY, fi->hfinfo->display);

    if (local_value_string && g_strcmp0(local_value_string, "") != 0)
    {
        gboolean add_field = FALSE;

        // 根据协议名和字段名确定是否添加字段
        if (g_strcmp0(protocol_name, "eth") == 0)
        {
            if (g_strcmp0(fi->hfinfo->abbrev, "eth.dst") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "eth.src") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "eth.type") == 0)
            {
                add_field = TRUE;
            }
        }
        else if (g_strcmp0(protocol_name, "ip") == 0)
        {
            if (g_strcmp0(fi->hfinfo->abbrev, "ip.src") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "ip.dst") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "ip.version") == 0)
            {
                add_field = TRUE;
            }
        }
        else if (g_strcmp0(protocol_name, "tcp") == 0)
        {
            if (g_strcmp0(fi->hfinfo->abbrev, "tcp.srcport") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "tcp.dstport") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "tcp.len") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "tcp.flags") == 0)
            {
                add_field = TRUE;
            }
        }
        ...
        {
            if (g_strcmp0(fi->hfinfo->abbrev, "udp.srcport") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "udp.dstport") == 0 ||
                g_strcmp0(fi->hfinfo->abbrev, "udp.length") == 0)
            {
                add_field = TRUE;
            }
        }
        else if (g_strcmp0(fi->hfinfo->abbrev, "data.data") == 0)
        {
            add_field = FALSE;
        }
        else
        {
            add_field = TRUE; // 其他协议的字段全部输出
        }
        ...
        if (add_field)
        {
            ...
            {
                label_ptr = label_str;
                proto_item_fill_label(fi, label_str, 0);
            }

            // 创建并填充ProtocolField结构体
            protocol_field field;
            field.name = g_strdup(fi->hfinfo->abbrev);
            field.showname = g_strdup(fi->hfinfo->name);
            field.size = g_strdup_printf("%d", fi->length);
            field.pos = g_strdup_printf("%d", fi->start);
            // field.show = fi->rep ? g_strdup(fi->rep->representation) : g_strdup("");
            field.value = g_strdup(local_value_string);
            field.show = g_strdup(label_ptr);
            // 将字段加入字段数组
            g_array_append_val(field_array, field);
        }
    }

    wmem_free(NULL, local_value_string);

    // 递归处理子字段
    proto_node *child = node->first_child;
    while (child != NULL)
    {
        write_field_data(child, field_array, protocol_name);
        child = child->next;
    }
}

static void
write_protocol_data(proto_node *node, GArray *protocol_array)
{
    if (node == NULL)
        return;
    ...
    {
        if (g_strcmp0(fi->hfinfo->abbrev, "frame") != 0)
        { // 忽略Frame协议
            protocol_data data;
            data.name = g_strdup(fi->hfinfo->abbrev);
            data.showname = g_strdup(fi->hfinfo->name);
            data.size = g_strdup_printf("%d", fi->length);
            data.pos = g_strdup_printf("%d", fi->start);
            data.fields = g_array_new(FALSE, FALSE, sizeof(protocol_field));

            // 处理字段级别
            proto_node *child = node->first_child;
            while (child != NULL)
            {
                write_field_data(child, data.fields, fi->hfinfo->abbrev);
                child = child->next;
            }

            // 将协议数据加入数组
            g_array_append_val(protocol_array, data);
        }
    }
    ...
    if (node->first_child)
    {
        write_protocol_data(node->first_child, protocol_array);
    }

    // 递归处理兄弟节点
    if (node->next)
    {
        write_protocol_data(node->next, protocol_array);
    }
}

static void
write_binary_proto_tree(epan_dissect_t *edt, GArray *protocol_array)
{
    proto_tree *tree = edt->tree;
    if (tree && tree->first_child)
    {
        write_protocol_data(tree->first_child, protocol_array);
    }
}
...
print_packet(capture_file *cf, epan_dissect_t *edt)
{
    ...
    case WRITE_EK:
        write_ek_proto_tree(output_fields, print_summary, print_hex, edt, &cf->cinfo, stdout);
        return !ferror(stdout);
    ...
    case WRITE_Binary:
        ...
        if (cinfo != NULL && cinfo->num_cols >= 7)
        {
            strncpy(pr.packet_info.proto, cinfo->columns[4].col_data, sizeof(pr.packet_info.proto) - 1);
            strncpy(pr.packet_info.sip, cinfo->columns[2].col_data, sizeof(pr.packet_info.sip) - 1);
            strncpy(pr.packet_info.dip, cinfo->columns[3].col_data, sizeof(pr.packet_info.dip) - 1);
            strncpy(pr.packet_info.desc, cinfo->columns[6].col_data, sizeof(pr.packet_info.desc) - 1);
        }
        else
        {
            strcpy(pr.packet_info.proto, "");
            strcpy(pr.packet_info.sip, "");
            strcpy(pr.packet_info.dip, "");
            strcpy(pr.packet_info.desc, "");
        }

        pr.protocol_info = g_array_new(FALSE, FALSE, sizeof(protocol_data));
        write_binary_proto_tree(edt, pr.protocol_info);
        // g_array_append_val(packet_array, pr);
...
void free_packet_result(packet_result *array)
{
    if (!array)
        return;

    for (guint i = 0; i < array->protocol_info->len; i++)
    {
        protocol_data *data = &g_array_index(array->protocol_info, protocol_data, i);
        g_free(data->name);
        g_free(data->showname);
        g_free(data->size);
        g_free(data->pos);

        for (guint j = 0; j < data->fields->len; j++)
        {
            protocol_field *field = &g_array_index(data->fields, protocol_field, j);
            g_free(field->name);
            g_free(field->showname);
            g_free(field->size);
            g_free(field->pos);
            g_free(field->show);
            g_free(field->value);
        }
        g_array_free(data->fields, TRUE);
    }
...
{
    capture_file local_cfile;
    wtap_rec local_rec;
    frame_data fdata;

    cap_file_init(&local_cfile);
    local_cfile.epan = tshark_epan_new(&local_cfile);

    epan_dissect_t *local_edt = epan_dissect_new(local_cfile.epan, true, true);

    wtap_rec_init(&local_rec, 1514);

    local_rec.rec_type = REC_TYPE_PACKET;
    local_rec.presence_flags = WTAP_HAS_TS | WTAP_HAS_CAP_LEN;
    local_rec.rec_header.packet_header.len = (guint32)len;
    local_rec.rec_header.packet_header.caplen = (guint32)len;
    local_rec.rec_header.packet_header.pkt_encap = WTAP_ENCAP_ETHERNET;

    ws_buffer_init(&local_rec.data, len);
    ws_buffer_assure_space(&local_rec.data, len);
    memcpy(ws_buffer_start_ptr(&local_rec.data), data, len);

    build_column_format_array(&local_cfile.cinfo, epan_load_settings()->num_cols, TRUE);

    column_info *cinfo = &local_cfile.cinfo;
    if (cinfo)
    {
    ...
    frame_data_init(&fdata, 0, &local_rec, 0, 0);
    frame_data_set_before_dissect(&fdata, &local_cfile.elapsed_time, &local_cfile.provider.ref, local_cfile.provider.prev_dis);
    epan_dissect_run(local_edt, local_cfile.cd_t, &local_rec, &fdata, cinfo);

    if (result)
    {
        if (cinfo != NULL && cinfo->num_cols >= 7)
        {
            strncpy(result->packet_info.proto, cinfo->columns[4].col_data, sizeof(result->packet_info.proto) - 1);
            strncpy(result->packet_info.sip, cinfo->columns[2].col_data, sizeof(result->packet_info.sip) - 1);
            strncpy(result->packet_info.dip, cinfo->columns[3].col_data, sizeof(result->packet_info.dip) - 1);
            strncpy(result->packet_info.desc, cinfo->columns[6].col_data, sizeof(result->packet_info.desc) - 1);
        }
    ...
    col_clear(&local_cfile.cinfo, TRUE);
    ws_buffer_free(&local_rec.data);
    frame_data_destroy(&fdata);
    epan_free(local_cfile.epan);
    epan_dissect_cleanup(local_edt);
    wtap_rec_cleanup(&local_rec);
    wtap_cleanup();
...
packet_result
dissect_single_packet(const uint8_t *data, size_t len)
{
    packet_result result = {0};

    dissect_packet_from_memory(data, len, &result);
